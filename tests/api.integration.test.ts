...
describe("bounded Action API",()=>{
 ...
 it("classifies malformed JSON as a validation failure",async()=>{const response=await POST(new Request("http://coach/api/v1/action",{method:"POST",body:"{"}));expect(response.status).toBe(400);expect(await response.json()).toMatchObject({ok:false,error:{code:"VALIDATION"}})});
 it("does not expose unexpected runtime failure details",async()=>{mocks.execute.mockRejectedValueOnce(new Error("sensitive connection detail"));const response=await POST(new Request("http://coach/api/v1/action",{method:"POST",body:JSON.stringify({operation:"listAthletes"})}));expect(response.status).toBe(500);expect(JSON.stringify(await response.json())).not.toContain("sensitive connection detail")});
});
